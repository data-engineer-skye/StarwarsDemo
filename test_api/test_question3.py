...
def test_get_all_vehicles_max_atmosphering_speed_over_1000(session):
    with allure.step("Get all vehicles data"):
        ...
        vehicles_api_obj.get_all_data()
        assert_check(vehicles_api_obj.get_response_status(), 200)
    ...
    with allure.step("Get max_atmosphering_speed > 1000 vehicles"):
        filter_vehicles = vehicles_api_obj.filter_vehicles_by_numeric_attribute("max_atmosphering_speed", "greater", 1000)
        logging.info(f'Match Vehicles: {filter_vehicles}')
        assert_check(filter_vehicles, expect_result)
